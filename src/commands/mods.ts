    ...
    try {
      if (subcommand === 'list') {
        ...
        if (subcommand === 'enable') {
          ...
          return interaction.editReply(`Successfully enabled mod: ${targetMod.name.replace('.disabled', '')}!`);
        } else {
          ...
          return interaction.editReply(`Successfully disabled mod: ${targetMod.name.replace('.jar', '')}!`);
        }
      }
    } catch (err) {
