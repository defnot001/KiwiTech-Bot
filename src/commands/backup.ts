  ...
  execute: async ({ interaction, args, client }) => {
    ...
    try {
      ...
      if (subcommand === 'create') {
        ...
        const backupName =
          args.getString('name') ?? `Backup created by ${client.user?.username} at ${new Date().toUTCString()}`;
        ...
        if (meta.pagination.total < backupLimit) {
          ...
          interaction.editReply(
            `Successfully created backup (${inlineCode(backup.name)}) for ${guild.name} ${bold(serverChoice)}!`,
          );
        ...
        collector.on('collect', async (i) => {
          if (i.customId === 'confirm') {
            ...
            interaction.editReply({
              content: `Successfully deleted oldest backup and created backup (${inlineCode(backup.name)}) for ${
                guild.name
              } ${bold(serverChoice)}!`,
              components: [],
            });
          ...
          interaction.editReply({
            content: `Cancelled deleting the oldest backup for ${guild.name} ${bold(serverChoice)}!`,
            components: [],
          });
        });
      ...
      if (!backupDetails) {
        interaction.editReply(
          `Could not find a backup with the name ${inlineCode(backupName)} for ${guild.name} ${bold(serverChoice)}!`,
        );
        return;
      }
      ...
      if (subcommand === 'delete') {
        if (!backupDetails.completed_at) {
          interaction.editReply(
            `Backup ${inlineCode(backupName)} for ${guild.name} ${bold(serverChoice)} is not completed yet!`,
          );
          return;
        }

        if (backupDetails.is_locked) {
          interaction.editReply(`Backup ${inlineCode(backupName)} for ${guild.name} ${bold(serverChoice)} is locked!`);
          return;
        }
        ...
        collector.on('collect', async (i) => {
          if (i.customId === 'confirm') {
            ...
            interaction.editReply({
              content: `Successfully deleted backup: ${inlineCode(backupDetails.name)} from ${guild.name} ${bold(
                serverChoice,
              )}!`,
              components: [],
            });
