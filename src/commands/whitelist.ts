    ...
    try {
      if (subcommand === 'list') {
        ...
        for await (const server of servers) {
          ...
          const whitelist = await runRconCommand(host, rconPort, rconPasswd, whitelistCommand);
          ...
          if (config.mcConfig[server as ServerChoice].operator) {
            ...
            const op = await runRconCommand(host, rconPort, rconPasswd, opCommand);
        ...
        const successMessage =
          subcommand === 'add'
            ? `Successfully added ${inlineCode(ign)} to the whitelist on ${
              ...
              } servers.\nSuccessfully made ${inlineCode(ign)} an operator on ${opCheck.length} servers.`
            : `Successfully removed ${inlineCode(ign)} from the whitelist on ${
                whitelistCheck.length
              } servers.\nSuccessfully removed ${inlineCode(ign)} as an operator on ${opCheck.length} servers.`;
