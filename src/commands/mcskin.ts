...
export default new Command({
  ...
  execute: async ({ interaction, args }) => {
    ...
    const imageType = args.getString('type', true) as '/avatars/' | '/renders/head/' | '/renders/body/' | '/skins/';
    ...
    try {
      const uuidResponse = await axios.get(`https://api.mojang.com/users/profiles/minecraft/${name}`);
